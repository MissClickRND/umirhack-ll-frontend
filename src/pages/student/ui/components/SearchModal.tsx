...
import { useAttachMyDiplomaMutation, useLazyGetDiplomaByIdQuery } from "@/entities/diploma";
...
export default function SearchModal({ opened, close, onAttached }: SearchModalProps) {
    const [value, setValue] = useState("");
    const [searchDiploma, { isFetching: isSearching }] = useLazyGetDiplomaByIdQuery();
    const [attachMyDiploma, { isLoading: isAttaching }] = useAttachMyDiplomaMutation();
    ...
    const handleAttachDiploma = async () => {
        ...
        try {
            ...
            const diploma = await searchDiploma(number).unwrap();
            ...
            notifications.show({
                title: "Ошибка",
                message: "Не удалось привязать диплом. Проверьте номер и попробуйте снова",
                color: "red",
            });
        }
    };
